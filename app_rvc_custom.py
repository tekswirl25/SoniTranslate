...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.error(f"[APP] step2a failed: {e}")
        models_path = index_path = None

    # step2b core
    logger.info("[APP] step2b: initializing core (SoniTranslate)…")
    try:
        ...
        logger.warning(f"[APP] init with models failed ({e}); retrying without models_path/index_path")
        SoniTr = SoniTranslate(
            cpu_mode=(args.cpu_mode if os.environ.get("ZERO_GPU") != "TRUE" else "cpu")
        )
    logger.info("[APP] step2b: core initialized")
